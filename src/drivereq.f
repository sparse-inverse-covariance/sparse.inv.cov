     ...
     +              igf,igg,jgg,nsel)
      ...
      integer MAXIT,LSI,I, ISTARTIT,NC,nr,nsel
...
c compute xr and gr assuming initial values specifed via inverse cov matrix
      if(nsel.eq.1)then
      call atmux(n,nr,x,xr,gg,jgg,igg)
      else if(nsel.eq.2)then
      call si2rho(x,gam,ia0,ja0,p)
...
c convert to gr
            if(nsel.eq.1)then
            call atmux(n,nr,bbb,gr,gg,jgg,igg)
            else if(nsel.eq.2)then
            call dldrcalc(gam,ia0,ja0,p,bbb,g)
