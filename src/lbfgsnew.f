...
C
     ...
     1                  ,G1,UNDEF,LSI,ITER,NFUN,GNORM,STP)
C
C  G1 STORES THE DERIVATIVE G IN THE LINE SEARCH ROUTINE LCSRCHQ1
C  LSI=0 FOR CRUDE LINESEARCH =1 FOR QUADRATIC INTERPOLATION
      INTEGER N,M,IPRINT(2),IFLAG
      DOUBLE PRECISION X(N),G(N),DIAG(N),W(N*(2*M+1)+2*M),G1(N)
      DOUBLE PRECISION F,EPS,XTOL,UNDEF
      LOGICAL DIAGCO
C
...
C     ----------------------------------------------------
 ...
 172  CONTINUE
      IF(LSI.EQ.0)then
      CALL LCSRCH(N,X,F,G,W(ISPT+POINT*N+1),STP,FTOL,
     *            XTOL,MAXFEV,INFO,NFEV,DIAG)
      ELSE
      CALL LCSRCHQ1(N,X,F,G,W(ISPT+POINT*N+1),STP,FTOL,
     *            XTOL,MAXFEV,INFO,NFEV,DIAG,G1,UNDEF)
      ENDIF
...
C
C     TERMINATION TEST  -changed by hk to max(abs(g))<eps
C     ----------------
C
...
C
      IF(IPRINT(1).GE.0) CALL LB1NEW(IPRINT,ITER,NFUN,
     *               GNORM,N,M,X,F,G,STP,FINISH)
      IF (FINISH) THEN
         IFLAG=0
         RETURN
...
C
 10   FORMAT('*************************************************')
 20   FORMAT('  N=',I5,'   NUMBER OF CORRECTIONS=',I5,
     .       /,  '       INITIAL VALUES')
 30   FORMAT(' F= ',1PD10.3,'   GNORM= ',1PD10.3)
 40   FORMAT(' VECTOR X= ')
 50   FORMAT(6(2X,1PD10.3))
 60   FORMAT(' GRADIENT VECTOR G= ')
 70   FORMAT(/'   I   NFN',6X,'FUNC',14X,'MAX GRADIENT',
     1      6X,'STEPLENGTH'/)
 80   FORMAT(2(I5,1X),3X,3(1PD16.9,2X))
...
C
      ...
      COMMON /LB3/MP,LP,GTOL,STPMIN,STPMAX
      DATA MP,LP,GTOL,STPMIN,STPMAX/6,6,9.0D-01,1.0D-20,1.0D20/
      END
C
C
C   ----------------------------------------------------------
